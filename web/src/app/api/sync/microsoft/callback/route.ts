...
import { exchangeCode, syncMicrosoftCalendarForUser } from "@/lib/sync/microsoft-calendar";
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return html("Microsoft Calendar connection cancelled", "Microsoft returned an error before Toatre could connect your calendar.");
  }

  if (!code || !state) {
    return html("Microsoft Calendar connection failed", "The Microsoft callback was missing a code or state value.", 400);
  }
  ...
  if (!stateDoc || !(stateDoc.expiresAt instanceof Date) || stateDoc.expiresAt < new Date()) {
    return html("Microsoft Calendar connection expired", "Please return to Toatre and try connecting Microsoft Calendar again.", 400);
  }
  ...
  try {
    ...
    return html("Microsoft Calendar connection failed", "Toatre could not complete the Microsoft token exchange. Please try again.", 400);
  }
  ...
  await settings.updateOne(
    ...
    {
      $set: {
        "syncConnections.microsoft": { provider: PROVIDER, direction, connected: true, connectedAt, forwardOnlyFrom, updatedAt: now },
        updatedAt: now,
      },
      $setOnInsert: { userId },
    },
  ...
  try {
    ...
    console.error("[microsoft/callback] initial sync failed", syncErr);
  }

  const returnTo = typeof stateDoc.returnTo === "string" && stateDoc.returnTo.startsWith("/") ? stateDoc.returnTo : "/settings?sync=microsoft";
