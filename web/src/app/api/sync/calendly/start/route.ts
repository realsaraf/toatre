...
import { calendlyAuthUrl } from "@/lib/sync/calendly";
...
export async function POST(request: NextRequest) {
  if (process.env.NEXT_PUBLIC_SYNC_CALENDLY_ENABLED !== "true") {
    return NextResponse.json({ error: "Calendly sync is not enabled." }, { status: 503 });
  }
  ...
  const authUrl = calendlyAuthUrl(state);
  return NextResponse.json({ authUrl, expiresAt: expiresAt.toISOString() });
}

function sanitizeReturnTo(value: string | undefined): string {
  if (!value || !value.startsWith("/")) return "/settings?sync=calendly";
  return value;
}
