...
import { exchangeCode, syncZoomForUser } from "@/lib/sync/zoom";
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return html("Zoom connection cancelled", "Zoom returned an error before Toatre could connect your account.");
  }

  if (!code || !state) {
    return html("Zoom connection failed", "The Zoom callback was missing a code or state value.", 400);
  }
  ...
  if (!stateDoc || !(stateDoc.expiresAt instanceof Date) || stateDoc.expiresAt < new Date()) {
    return html("Zoom connection expired", "Please return to Toatre and try connecting Zoom again.", 400);
  }
  ...
  try {
    ...
    return html("Zoom connection failed", "Toatre could not complete the Zoom token exchange. Please try again.", 400);
  }
  ...
  await calendarSyncTokens.updateOne(
    { userId, provider: PROVIDER },
    {
      $set: {
        ...
        scope: "meeting:read:list_meetings user:read",
        connectedAt,
        forwardOnlyFrom,
        updatedAt: now,
      },
  ...
  await settings.updateOne(
    ...
    {
      $set: {
        "syncConnections.zoom": { provider: PROVIDER, direction, connected: true, connectedAt, forwardOnlyFrom, updatedAt: now },
        updatedAt: now,
      },
      $setOnInsert: { userId },
    },
  ...
  try {
    await syncZoomForUser(userId);
  } catch (syncErr) {
    console.error("[zoom/callback] initial sync failed", syncErr);
  }

  const returnTo = typeof stateDoc.returnTo === "string" && stateDoc.returnTo.startsWith("/") ? stateDoc.returnTo : "/settings?sync=zoom";
