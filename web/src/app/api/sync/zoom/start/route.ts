...
import { zoomAuthUrl } from "@/lib/sync/zoom";
...
export async function POST(request: NextRequest) {
  if (process.env.NEXT_PUBLIC_SYNC_ZOOM_ENABLED !== "true") {
    return NextResponse.json({ error: "Zoom sync is not enabled." }, { status: 503 });
  }
  ...
  const authUrl = zoomAuthUrl(state);
  return NextResponse.json({ authUrl, expiresAt: expiresAt.toISOString() });
}

function sanitizeReturnTo(value: string | undefined): string {
  if (!value || !value.startsWith("/")) return "/settings?sync=zoom";
  return value;
}
